...
For users pulling the package, no separate manual classifier installation is
required after Docker Desktop is installed and running. The root `./start.sh`
script asks Docker Compose to build and start this sidecar automatically. For
teachers or reviewers, this document explains why the classifier is isolated and
how Layer 08 obtains subjectivity/objectivity scores.
