...
If neither the Docker sidecar nor the local package path is available, Layer 08
uses deterministic sentence-level marker and phrasing features. The result still
contains subjectivity/objectivity scores and sentence metadata, but the detector
source is recorded as `deterministic_subjectivity_fallback`.
